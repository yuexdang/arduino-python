...
# 与外置舵机映射角度
def padding_center(mov_center,cv_center=cv_center):
    height_center = cv_center[0] / 2
    width_center = cv_center[1] / 2
    height_difference = (  mov_center[0] - height_center ) / cv_center[0]
    width_difference = ( - mov_center[1] + width_center ) / cv_center[1]
    # print( bytearray(np.array([round(round(height_difference,3)*10,2),round(round(width_difference,3)*10,2)])) )
    print("高度偏移：{}，宽度偏移：{}".format(round(round(height_difference,3)*10,2)*12,round(round(width_difference,3)*10,2)*12))
    pin7.write(-round(round(height_difference,3)*10,2)*12+60)
    pin10.write(-round(round(width_difference,3)*10,2)*12+50)
...
class Face_Recognizer:
    ...
    def show_chinese_name(self):
        ...
        if self.current_frame_face_cnt >= 1:
            ...
            self.name_known_list[0] = '这个用不上'.encode('utf-8').decode()
    ...
    def process(self, stream):
        ...
        if self.get_face_database():
            while stream.isOpened():
                    ...
                    if self.current_frame_face_cnt == 1:
                        if self.reclassify_interval_cnt == self.reclassify_interval:
                            ...
                            for k, d in enumerate(faces):
                                ...
                                hh = int(height / 2)
                                ww = int(width / 2)
                                # print(d.left()+ww,d.top()+hh)
                                angle_percent = padding_center((d.left()+ww,d.top()+hh))
                ...
                # print(cv2.getWindowImageRect('camera'))  #显示opencv视频流信息
